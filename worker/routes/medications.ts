...
import { validateTimeArray, validateText } from '../shared/contentLimits'
import type { MedicationInput } from '../../shared/types'
import { isDueOnDate, scheduledInstantsFor, dateStrInTimezone } from '../../shared/scheduling'
...
async function backfillDoseLogs(
  ...
  while (dateStrInTimezone(cursor, timeZone) < today) {
    ...
    if (isDueOnDate(medication, dateStr, timeZone)) {
      for (const scheduledFor of scheduledInstantsFor(medication, dateStr, timeZone)) {
        rows.push({
          userId,
          medicationId,
          scheduledFor,
...
app.all('/api/medications', async (c) => {
  const req = c.req.raw
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const timesError = validateTimeArray(body.preferredTimes, 'preferredTimes')
      if (timesError) return jsonResponse({ error: timesError }, { status: 400 })
