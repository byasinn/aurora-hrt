...
import { routineLogs, routines } from '../../shared/schema'
...
app.all('/api/routine-logs', async (c) => {
  const req = c.req.raw
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const [routine] = await db
        ...
        .from(routines)
        .where(and(eq(routines.id, body.routineId), eq(routines.userId, user.id)))
        .limit(1)
      if (!routine) return jsonResponse({ error: 'rotina não encontrada' }, { status: 404 })
      const [row] = await db
        .insert(routineLogs)
